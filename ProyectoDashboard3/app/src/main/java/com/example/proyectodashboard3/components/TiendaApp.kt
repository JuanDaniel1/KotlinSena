...
import com.example.proyectodashboard3.pages.login.LoginScreen
import com.example.proyectodashboard3.pages.splash.SplashScreen
...
fun TiendaApp(
    ...
    val videoUri = "android.resource://com.example.proyectodashboard3/raw/clouds"
    Uri.parse(videoUri)
    ...
    Scaffold(
        ...
        NavHost(navController = navController, startDestination = PagesScreen.Start.name){
            ...
            composable(route = PagesScreen.Tienda.name){
                index = false
                MainPage(state = index)
            }
